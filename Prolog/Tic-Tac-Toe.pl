% To play a game with the computer, type play
% Driving Code:- play.
% 
...
output([A,B,C,D,E,F,G,H,I]) :- write([A,B,C]),nl,
  							   write([D,E,F]),nl,
							   write([G,H,I]),nl,nl.
...
orespond(Board,Newboard) :- not(member(b,Board)),
                            !, 
                            write('Draw game!'), nl,
                            Newboard = Board.
...
explain :-write('You play X by entering integer positions followed by a period.'),nl,
  			output([1,2,3,4,5,6,7,8,9]).

playfrom(Board) :- win(Board, x), write('You win!').
playfrom(Board) :- win(Board, o), write('Computer win!').
